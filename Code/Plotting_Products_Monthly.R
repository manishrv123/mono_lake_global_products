...
precip = c(global_precip,length(y))

yearly = seq(from = as.Date("1895-01-01"), to = as.Date("2023-10-01"), by = 'year')
for (i in seq(1,length(yearly)-10,by=10)) {
  startdate = as.character(yearly[i])
  enddate =as.character(yearly[i+10])
  plotname=paste("Plots//","Products_Monthly_",startdate,"_",enddate,".pdf",sep="") #Comment to SIP3
  ...
  for (i in precip) {
    lines(as.Date(y$Date[s_m:e_m]), as.numeric(y[,i][s_m:e_m]), col = cols[legend_counter],  lwd = 1)
  ...
  plot(c(0),c(0),xlab="Date", ylab="Evap Rate (In)", type = "n", col = 8, lwd = 3, 
       axes = FALSE,ylim=c(0,20),xlim=c(as.Date(startdate),as.Date(enddate)))
  legend=array(NA)
  legend_counter=1
  for (i in evap) {
    lines(as.Date(y$Date[s_m:e_m]), as.numeric(y[,i][s_m:e_m]), col = cols[legend_counter],  lwd = 1)
